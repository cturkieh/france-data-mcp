...
- **Territory health snapshot (1) — V0.9**: `panorama_sante_territoire` — single-call aggregator (population + multi-PS densities vs national + FINESS counts per family + **IRIS demand block**: commune demographic profile — age, occupation, families, income — aggregated from its IRIS)
...
✅ **v0.22.0 — in production.** Listed on the [official MCP Registry](https://registry.modelcontextprotocol.io/v0.1/servers?search=france-data-mcp). Details: [CHANGELOG](CHANGELOG.md).
