...
- **Fallback SIRENE INSEE V3 (v0.4.4)** : `entreprise_by_siren` tente automatiquement un lookup via SIRENE INSEE V3.11 quand DINUM ne connaît pas le SIREN (cas diffusion partielle INSEE — ex: BIO ARD'AISNE 787120435). Activé via `INSEE_SIRENE_CLIENT_ID` / `INSEE_SIRENE_CLIENT_SECRET` (inscription gratuite sur portail-api.insee.fr). No-op gracieux sans creds. Le champ `siren_source` (`"dinum"` | `"insee_v3"`) trace la provenance.
