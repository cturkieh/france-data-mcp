...
- **Index fonctionnel Unicode-lourd dans `ingest_create_*_staging` + UPDATE de masse pré-swap = timeout 57014** (post-mortem désamorçage BAN 2026-05-18). Un index fonctionnel sur une normalisation Unicode (`rpps_address_key_for_index`) avec prédicat partiel sur des colonnes que `ingest_apply_*_finess_enrichment_batch` (UPDATE de masse) modifie ⇒ maintenance d'index = recalcul Unicode par ligne × 100k+ → `statement_timeout` (57014) en phase `validate`, AVANT le swap (données intactes mais cron cassé chaque mois, « tout seul »). Tout index BAN/Unicode doit vivre dans un step DÉDIÉ post-enrichment, JAMAIS dans `ingest_create_*_staging`.
