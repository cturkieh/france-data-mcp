...
- **Ré-armement BAN : les 2 index BAN sont créés par la RPC dédiée `ingest_build_rpps_staging_ban_indexes()` sur `rpps_staging`, APRÈS l'enrichment FINESS et AVANT le swap — JAMAIS dans `ingest_create_rpps_staging`, JAMAIS post-swap sur la table `rpps` servie** (refonte 2026-05-18, plan `docs/plans/2026-05-18-ban-rearm-postenrichment-index-step.md`). Pourquoi : (a) dans staging-create → maintenus pendant l'INSERT 2,24 M + l'UPDATE enrichment = re-allonge le run (l'AGGRAVANT, cf. gotcha ci-dessus) ; (b) post-swap sur `rpps` servie → `CREATE INDEX` non concurrent bloque les lectures prod, et `CONCURRENTLY` exige une connexion directe hors pooler (surface inutile). La voie correcte (doc PostgreSQL « Populating a Database » : indexer APRÈS le chargement) = build unique post-enrichment sur `rpps_staging` (qui ne sert AUCUNE lecture prod → `CREATE INDEX` bloquant classique sans impact externe ; `CONCURRENTLY` interdit en plpgsql de toute façon) ; les index voyagent dans `rpps` via le RENAME du swap, et sont reconstruits à chaque cron (le swap suivant détruit l'OID — coût accepté, ~10 min/mois). Séquence load-bearing dans `scripts/ingest/rpps.ts` : analyze → enrichment → `ingest_build_rpps_staging_ban_indexes` (fail-loud) → re-analyze → `runBanGeocodeStep('rpps_staging')` (best-effort) → swap → rebuildMatviews. Expression `rpps_address_key_for_index(...)` + prédicat byte-identiques RPC↔migrations↔count (gardés par `ban-eligibility-*-parity.test.ts` ; jumeau SQL/JS NON modifié — parité par transitivité).
