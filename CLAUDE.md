...
**Canal d'apply des migrations en prod (V0.12.3)** : **dashboard SQL editor manuel** (https://supabase.com/dashboard/project/_/sql/new). Les migrations format `YYYYMMDDTHHMMSS_*.sql` (depuis V0.4.4, majorité du repo) sont **rejetées par `supabase db push`** (pattern CLI exige `YYYYMMDDHHMMSS_` sans T) — d'où la suppression de `pnpm db:push` du `package.json` en V0.12.3 (alias mort). L'auto-apply via GitHub Actions a été audité et parqué — voir mémoire `migration-autoapply-deferred`. En attendant : coller le SQL de la nouvelle migration dans le dashboard avant ou immédiatement après le push code. Le retry défensif `PGRST204_RECOVERABLE_FIELDS` protège la fenêtre transitoire (cf. ci-dessous).
